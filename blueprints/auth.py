...
from finance_app.lib.auth import _get_csrf_token, csrf_failure_response, csrf_token_valid
...
def _finalize_logout(user_id: int | None) -> None:
    if not user_id:
        return
...
def forgot_username():
    if request.method == 'POST':
        ...
        if not email:
            flash('Email is required.')
            return render_template('forgot_username.html', csrf_token=_get_csrf_token())
...
def forgot_password():
    if request.method == 'POST':
        ...
        if not username or not email or not new_password:
            ...
            return render_template('forgot_password.html', csrf_token=_get_csrf_token())
